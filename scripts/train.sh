...
: ${DATASET_PATH:="/data2/tuong/OLLI-SPEECH-1.8/LJSpeech_style"}
: ${TRAIN_FILELIST:=filelists/filelist_train_pitch.txt}
: ${VAL_FILELIST:=filelists/filelist_val_pitch.txt}
: ${AMP:=false}
: ${SEED:="1234"}
...
if [ "$SAMPLING_RATE" == "44100" ]; then
  ...
  ARGS+=" --sampling-rate 22050"
  ARGS+=" --filter-length 1024"
  ARGS+=" --hop-length 256"
  ARGS+=" --win-length 1024"
  ARGS+=" --mel-fmin 0.0"
  ARGS+=" --mel-fmax 11025.0"
fi
